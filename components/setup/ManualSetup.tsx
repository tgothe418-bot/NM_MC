    ...
    <div className="flex flex-col h-full overflow-hidden bg-[#030303]">
        ...
        <div className="flex-1 p-12 md:p-16 space-y-12 overflow-y-auto custom-scrollbar">
            <div className="flex justify-between border-b border-fresh-blood/30 pb-10">
                <div className="flex gap-6"><ShieldAlert className="w-14 h-14 text-fresh-blood animate-pulse" /><div><h2 className="font-mono text-5xl font-bold tracking-[0.25em] text-fresh-blood">MANUAL SETUP</h2><p className="text-sm font-mono text-gray-500 tracking-[0.4em] mt-2">Configuration Mode</p></div></div>
            ...
            {store.parsedCharacters.length > 0 && (
                <div className="space-y-6 animate-fadeIn">
                    <div className="flex items-center gap-3 text-sm font-mono uppercase tracking-[0.3em] text-gray-400 border-b border-gray-800 pb-2"><Users className="w-4 h-4" /> Characters</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16">
                ...
                <div className="space-y-4"><label className="text-sm font-mono text-gray-400 flex gap-3"><Hourglass className="text-fresh-blood" /> Length</label><select value={store.startingPoint} onChange={(e) => store.setStartingPoint(e.target.value)} className="w-full bg-black border-2 border-gray-800 p-6 text-lg"><option>Prologue</option><option>In Media Res</option><option>The Climax</option></select></div>
                
                <div className="col-span-full space-y-6"><label className="text-sm font-mono text-gray-400 flex gap-3"><Settings className="text-fresh-blood" /> Aesthetic</label><div className="grid grid-cols-7 gap-4">{CLUSTER_OPTIONS.map(o => (<button key={o.id} onClick={() => store.setSelectedClusters(prev => prev.includes(o.id) ? prev.filter(c => c !== o.id) : [...prev, o.id])} className={`p-5 border-2 text-left transition-all ${store.selectedClusters.includes(o.id) ? `${o.color} bg-black ring-2 ring-fresh-blood/50` : 'border-gray-800 text-gray-600'}`}><div className="font-bold text-xs uppercase">{o.label}</div><div className="text-[10px] opacity-60">{o.desc}</div></button>))}</div></div>
                
                <div className="col-span-full bg-terminal/40 p-10 border border-gray-800 space-y-10 rounded-sm">
                    <div className="text-fresh-blood font-mono text-xl font-bold uppercase tracking-widest border-b border-fresh-blood/10 pb-5 flex gap-5"><MapPin className="w-8 h-8" /> Location</div>
                    <div className="grid grid-cols-2 gap-12">
                        <div className="space-y-4"><div className="flex justify-between"><label className="text-sm font-mono text-gray-400">Description</label><button onClick={() => handleGenerate('Initial Location')} className="text-gray-500 hover:text-fresh-blood">{loadingFields['Initial Location'] ? <Loader2 className="animate-spin" /> : <Sparkles />}</button></div><textarea value={store.locationDescription} onChange={e => store.setLocationDescription(e.target.value)} className="w-full h-32 bg-black border-2 border-gray-800 p-6 text-sm outline-none resize-none" /></div>
