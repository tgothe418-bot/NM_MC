...
export const useGameEngine = () => {
    ...
    const handleSendMessage = useCallback(async (text: string, files: File[] = [], overrideState?: GameState) => {
        ...
        try {
            ...
            const isRateLimit = e?.message === "RATE_LIMIT_EXCEEDED" || e?.status === 429 || e?.message?.includes('429') || e?.message?.includes('Rate Limit');
            ...
            setHistory(prev => [...prev, { 
                role: 'model', 
                text: isQuota
                    ? "SYSTEM HALT: Cognitive Overload (Quota Exceeded). Please provide a valid API key or wait until your quota resets."
                    : isRateLimit 
