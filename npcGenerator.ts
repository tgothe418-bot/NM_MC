...
// --- INITIALIZATION (Singleton Pattern Removed) ---

export const initializeGemini = (apiKey: string) => {
    // No-op, kept for backward compatibility if needed
};
 ...
 */
const withRetry = async <T>(fn: () => Promise<T>, maxRetries = 5, initialDelay = 2000, timeoutMs = 30000): Promise<T> => {
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                isRateLimit = String(e.message || '').includes('429') || 
                             ...
                             e.code === 429;
                isQuota = String(e.message || '').includes('quota');
                isTimeout = String(e.message || '') === 'TIMEOUT';
            }
            
            if ((isRateLimit || isTimeout) && !isQuota && i < maxRetries - 1) {
                const delay = initialDelay * Math.pow(2, i);
                console.warn(`Gemini API Issue (${isTimeout ? 'Timeout' : 'Rate Limit'}). Retrying in ${delay}ms... (Attempt ${i + 1}/${maxRetries})`);
